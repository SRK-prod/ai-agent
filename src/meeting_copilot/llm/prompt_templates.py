...
_CATEGORY_SHAPES: dict[str, str] = {
    ...
    "cicd_devops": (
        ...
        "    * Your pipeline philosophy in one line.\n"
        "  ## Pipeline Flow\n"
        "    A compact ASCII flow, adapted to what is actually relevant:\n"
        "    Git -> Build -> Test -> Scan -> Artifact -> Non-Prod -> Validate -> Approve -> "
        "Prod -> Observe\n"
        "  ## Source and Branching\n"
        "    * Branching and trigger model -- trunk-based, PR checks, what starts a build.\n"
        "    * Where the pipeline definition lives and how it is versioned with the code.\n"
        "  ## Build and Test\n"
        "    * What the build produces and how it is made reproducible.\n"
        "    * What actually runs at each stage -- unit, integration, contract, smoke -- and "
        "which of them can block a release.\n"
        "  ## Security Gates\n"
        "    * SAST, SCA, container image scanning, secret scanning, IaC scanning -- name the "
        "tools and say which findings break the build rather than just warn.\n"
        "    * Image signing and provenance, and what admission control enforces at deploy.\n"
        "  ## Artifacts\n"
        "    * Immutable, versioned artifacts in a real registry -- built once, promoted, "
        "never rebuilt per environment.\n"
        "    * Retention and traceability back to the commit that produced them.\n"
        "  ## Environments and Promotion\n"
        "    * The environment path and what must be true to promote between them.\n"
        "    * How environment-specific config is injected without rebuilding the artifact.\n"
        "  ## Secrets in the Pipeline\n"
        "    * Where pipeline credentials live and how they are scoped per environment.\n"
        "    * OIDC/short-lived cloud credentials rather than long-lived static keys.\n"
        "  ## Deployment Strategy\n"
        "    * Rolling, blue-green or canary -- pick one and say why for this workload.\n"
        "    * The health signal that decides whether the rollout continues.\n"
        "  ## Approvals and Governance\n"
        "    * What gates production specifically, who or what approves, and the audit trail.\n"
        "    * Separation of duties where the environment is regulated.\n"
        "  ## Rollback\n"
        "    * The concrete rollback path -- previous artifact, previous infra version.\n"
        "    * The measured trigger that fires it automatically, not a judgement call.\n"
        "  ## Observability of Delivery\n"
        "    * Deployment metrics that matter -- lead time, change failure rate, MTTR.\n"
        "    * How a deploy event is correlated with the error-rate and latency it caused.\n"
        "  ## Scale Across Teams\n"
        "    * Reusable templates or shared workflows so 50 teams do not write 50 pipelines.\n"
        "    * The extension point that lets a team differ without forking the standard.\n"
        "  ## Principal Architect Decision\n"
        "    * How you balance standardization, team autonomy, governance and delivery speed "
