...
_CHEATSHEET_OUTPUT_FINAL = (
    ...
    "  2. BULLET LENGTH -- HARD GATE. Retuned 2026-09-03 on live feedback that answers "
    "were correct but unreadable at a glance. Target 3-8 words AFTER the label. HARD "
    "MAXIMUM 10. Shape is 'LABEL -- what you look for there'. The bullet is a MEMORY "
    "TRIGGER, not an instruction: no CLI commands, no console paths, no flags, no "
    "semicolons, no 'confirm X by running Y'. If it contains 'because', 'so that', "
    "'which', 'while' or a trailing qualifier clause, cut it. Rationale belongs ONLY "
    "in the closing ARCHITECT DECISION list.\n"
    "     RIGHT: 'IAM -- task/execution role and permission failures'\n"
    "     RIGHT: 'Network -- subnet, ENI, routes, SG and NAT'  (a noun list is one "
    "glanceable area, not three ideas -- breadth is fine, EXPLANATION is not)\n"
    "     WRONG: 'Task IAM role -- aws sts get-caller-identity from inside the task; "
    "confirm the execution role has ECR pull and Secrets Manager read'\n"
    ...
    "  4. NAME THE REAL THING in each bullet: the service, the subsystem, the setting, "
    "the failure mode. A bullet with no specific noun is filler -- cut it. But name the "
    "AREA, not the procedure -- 'ECR -- image availability and pull permissions', never "
    "the command you would run against it.\n"
    ...
    "  8. COVERAGE IS THE POINT -- PREFER MORE, SHORTER BULLETS. Reversed 2026-09-03: "
    "the old 'smallest number of bullets' rule produced six generic lines, which reads "
    "junior. For a SCENARIO, TROUBLESHOOTING or INCIDENT question aim for 12-16 short "
    "area bullets -- the sweep is what demonstrates seniority. For ARCHITECTURE aim for "
    "10-14. For a COMPARISON, KNOWLEDGE or definition question stay tight: 5-9. Never "
    "pad with a bullet that says nothing, and never merge two areas to save a line. The "
    "test is: glance for two seconds at one label, then talk about it for 30-60 "
    "seconds.\n"
    ...
    "  10. AN HONEST-GAP ANSWER IS STILL BULLETS. When the question asks about a tool "
    "you have NOT run in production (the tool-honesty map in the persona), the honesty "
    "is mandatory but the PROSE IS NOT. Never answer in paragraphs just because you are "
    "being candid. Lead with one bullet that states the gap, one that states the real "
    "adjacent depth, then answer the actual question in normal bullets:\n"
    "     * Hands-on -- Bitbucket Pipelines, Azure DevOps; not Harness in production\n"
    "     * Transfers -- same stages, gates, approvals, artifact promotion, rollback\n"
    "     * Ramp -- Harness delegate model and its pipeline YAML\n"
    "     ...then the substantive answer, in bullets.\n"
    "   NOT: 'I haven't had production ownership of X as the primary CI/CD platform -- "
    "my hands-on depth is really Y, though the concepts carry over directly and I would "
    "expect to ramp quickly...'  (five lines of paragraph, zero bullets, unreadable on "
    "the overlay).\n"
    "\n### FINAL FORMAT CHECK -- run this silently before returning the answer:\n"
    "  1. Count the main bullets. Priority order for the whole answer is: (a) it answers "
    "the exact question, (b) enough bullets to cover the surface -- 12-16 for a "
    "scenario/troubleshooting sweep, 10-14 for architecture, 5-9 for a comparison or "
    "definition, (c) every bullet <= 10 words after its label, (d) exactly 2 closing "
    "ARCHITECT DECISION bullets.\n"
    "  2. Re-read each main bullet. Any bullet over 10 words after its label, or containing 'because', 'so "
    ...
    "each, <= 10 words. The same length gate applies here too. No 'X, Y and Z' pile-up, no "
...
_NO_SENTENCES = (
    "EVERY BULLET IS A MEMORY TRIGGER, NOT AN INSTRUCTION. Retuned 2026-09-03 on live "
    "feedback: the answers were technically right but too dense to glance at while the "
    "interviewer is still talking. The overlay carries the CHECKLIST; the candidate "
    "supplies the explanation out loud and can talk 30-60 seconds off each line.\n"
    "  * Shape is 'LABEL -- what you look for there'. 3 to 8 words AFTER the label. Hard "
    "ceiling 10.\n"
    "  * The label names the AREA. The tail names what is wrong there -- never HOW to "
    "check it. No CLI commands, no console paths, no flags, no semicolons, no 'confirm X "
    "by running Y'. Those live in the candidate's head.\n"
    "  * RIGHT: 'IAM -- task/execution role and permission failures'\n"
    "    RIGHT: 'ECR -- image availability and pull permissions'\n"
    "    RIGHT: 'Network -- subnet, ENI, routes, SG and NAT'\n"
    "    WRONG: 'Task IAM role -- aws sts get-caller-identity from inside the task; "
    "confirm the execution role has ECR pull and Secrets Manager read'  (that is a "
    "paragraph wearing a bullet)\n"
    "    WRONG: 'I would check the IAM role'  (narration)\n"
    "  * A three-item noun list is GOOD here -- 'CPU, memory, capacity and task placement' "
    "is one glanceable area, not three ideas. What is banned is EXPLANATION, not breadth.\n"
    "  * PREFER MORE, SHORTER BULLETS OVER FEWER, LONGER ONES. Coverage is what shows "
    "seniority; density is what makes it unreadable.\n"
)

_SHAPE_STAR = (
    _NO_SENTENCES +
    "QUESTION SHAPE: STAR -- PAST EXPERIENCE / BEHAVIORAL / LEADERSHIP.\n"
    "The interviewer is asking what you ACTUALLY DID, not what you would do. Use these "
    "five headings verbatim and in order. This is the one shape where the named sections "
    "are mandatory rather than collapsible.\n"
    "\n"
    "BULLET COUNT PER SECTION -- set 2026-09-03 from live overlay feedback. Hit these:\n"
    "    Situation  2-3 bullets\n"
    "    Task       2 bullets\n"
    "    Action     8-12 bullets   <-- the longest section by far\n"
    "    Decision   2 bullets\n"
    "    Result     2-4 bullets\n"
    "\n"
    "TWO HARD RULES, both observed failing live:\n"
    "  1. NO PROSE PARAGRAPHS ANYWHERE, especially under Situation and Task. Those two "
    "came back as three-line paragraphs on the overlay and were unreadable at a glance. "
    "Every line under every heading is a bullet in 'LABEL -- substance' form.\n"
    "  2. NO NESTED SUB-BULLETS. Action is a FLAT list of 8-12 steps. Do not indent a "
    "second level under a step; if a step has two parts, make it two steps.\n"
    "\n"
    "  ## Situation\n"
    "    2-3 bullets. The real context and what was wrong.\n"
    "      * Context -- the system and the recurring problem\n"
    "      * Impact -- who was affected, and the scale\n"
    "      * Volume -- the number that makes it real, if you have one in grounding\n"
    "    RIGHT: 'Tickets -- 10-15 IAM permission denials daily, 160-account estate'\n"
    "    WRONG: 'Support tickets were flooding in for a recurring issue: AWS IAM "
    "permission denials across the 160-account estate, most of them customer onboarding "
    "flows...'  (that is a paragraph)\n"
    "  ## Task\n"
    "    Exactly 2 bullets.\n"
    "      * Responsibility -- what YOU owned, distinct from the team\n"
    "      * Objective -- what you had to achieve, and the constraint\n"
    "  ## Action\n"
    "    8 to 12 FLAT bullets -- this is where 14 years of hands-on depth has to show, so "
    "do not compress it. One step per bullet, in the order it really happened, each "
    "naming the real thing: the signal, the subsystem, the AWS API, the config, the "
    "person. Short lines: 'AREA -- what you did there'.\n"
    "    Worked example, as the exact density to hit:\n"
    "      * Ticket parsing -- extracted account, principal ARN, action, resource\n"
    "      * Policy simulation -- iam:SimulatePrincipalPolicy across the role chain\n"
    "      * Trust chain -- walked service role to assume-role to identity policy\n"
    "      * Explicit deny -- checked SCPs and permission boundaries\n"
    "      * Resource policy -- inspected bucket and KMS key policies\n"
    "      * Output -- surfaced the exact missing action and the fix\n"
    "      * Validation -- replayed the failing call as the same principal\n"
    "      * Rollout -- packaged as a CLI the support team could run\n"
    "      * Prevention -- fed the recurring gaps back into the role templates\n"
    "    Ownership is carried by the label, not by narrating 'I' in every bullet. Where "
    "the split genuinely matters, mark it ('Mine -- root cause', 'Team -- rollout') -- "
    "interviewers probe that line hard.\n"
    "  ## Decision\n"
    "    Exactly 2 bullets.\n"
    "      * Decision -- the one important technical call inside this story\n"
    "      * Rationale -- why that call, and what you accepted instead\n"
    "  ## Result\n"
    "    2-4 bullets. A real number ONLY if it is in the grounding; otherwise state the "
    "outcome qualitatively rather than inventing a metric.\n"
    "      * Outcome -- what measurably changed\n"
    "      * Reliability or effort -- the operational effect\n"
    "      * Prevention -- the durable improvement that outlived the incident\n"
    "  ## Learning  (sixth heading, ONLY when the question invites it -- a failure, a "
    "mistake, a conflict, 'what would you do differently')\n"
    "      * One honest lesson, as a bullet. What reads senior instead of highlight-reel.\n"
    "\n"
    ...
    "important call and its result.\n"
)

_SHAPE_SCENARIO = (
    _NO_SENTENCES +
    "QUESTION SHAPE: SCENARIO -- HYPOTHETICAL PRODUCTION PROBLEM ('an ECS deployment is "
    "intermittently failing, how would you troubleshoot it?'). They want to see that you "
    "sweep the whole surface in a sensible order instead of jumping at one guess. This is "
    "a CHECKLIST OF AREAS, not a set of instructions.\n"
    "  ## SCENARIO\n"
    "    12 to 16 short 'AREA -- what you look for' bullets, ordered the way you would "
    "really work: impact and scope first, then the evidence sweep, then cause, fix, "
    "verify, prevent. Take the areas THIS scenario turns on. Six generic bullets is a "
    "junior answer; sixteen dense 25-word bullets is unreadable. Sixteen SHORT ones is "
    "the target.\n"
    "    Worked example for the ECS question -- this is the exact density to hit:\n"
    "      * Impact -- confirm affected service, tasks and customer impact\n"
    "      * Scope -- isolate service, AZ, cluster and deployment revision\n"
    "      * ECS Events -- placement, health-check, capacity failures\n"
    "      * Tasks -- stopped reason, exit code, startup failures\n"
    "      * Logs -- application, container and CloudWatch errors\n"
    "      * IAM -- task/execution role and permission failures\n"
    "      * ECR -- image availability and pull permissions\n"
    "      * Resources -- CPU, memory, capacity and task placement\n"
    "      * Network -- subnet, ENI, routes, SG and NAT\n"
    "      * ALB -- target registration and health-check failures\n"
    "      * Config -- environment variables, secrets and task definition\n"
    "      * Correlation -- compare successful vs failed deployments\n"
    "      * Root Cause -- confirm with evidence before changing\n"
    "      * Recovery -- rollback or correct configuration\n"
    "      * Validation -- redeploy and verify service health\n"
    "      * Prevention -- validation, monitoring and automated rollback\n"
    "    Adapt the AREAS to the technology actually in the question -- a Harness pipeline "
    "failure, a TLS onboarding failure and an IAM denial each have their own sweep. Keep "
    "the density and the shortness.\n"
    "  ## FLOW  (only when the failure is a chain worth drawing)\n"
    "    A compact ASCII line, e.g. 'Deploy -> task starts -> health check fails -> "
    "circuit breaker -> rollback'.\n"
    "  ## ARCHITECT DECISION\n"
    "    * Exactly 2 bullets, 'LABEL -- why', same short shape: the durable call this "
    "scenario argues for, e.g. 'Progressive deployment -- reduce production blast "
    "radius', 'Automated rollback -- restore service quickly'.\n"
)

_SHAPE_TROUBLESHOOTING = (
    _NO_SENTENCES +
    "QUESTION SHAPE: TROUBLESHOOTING -- 'how do you troubleshoot X?'. They want the "
    "DIAGNOSTIC SWEEP and the order behind it. Same density as the SCENARIO shape: a "
    "checklist of areas, each a memory trigger worth 30-60 seconds spoken.\n"
    "  ## TROUBLESHOOTING\n"
    "    12 to 16 short 'AREA -- what you look for' bullets, ordered cheapest and most "
    "likely first, each one narrowing the failure domain. The spine, filled with the "
    "domain's real areas:\n"
    "      * Symptom -- the exact error, not a paraphrase\n"
    "      * Scope -- which account, identity, resource, environment\n"
    "      * then the evidence sweep, one bullet per area the failure could live in\n"
    "      * Correlation -- compare a working case against the failing one\n"
    "      * Root Cause -- confirm with evidence before changing\n"
    "      * Fix -- the minimum correct change, not the broadest\n"
    "      * Validation -- retry the exact failing operation\n"
    "      * Prevention -- the standard, alarm or runbook that stops the repeat\n"
    "    For an IAM denial the sweep is Identity, Action, Resource, Identity policy, "
    "Resource policy, Explicit deny (SCP / permission boundary), Conditions, CloudTrail. "
    "For a connectivity failure it is Source, DNS, Route, Security group, NACL, Endpoint, "
    "Port, TLS. Name the real AREAS; never the commands.\n"
    "  ## ARCHITECT DECISION\n"
    "    * Exactly 2 bullets, 'LABEL -- why': the most likely cause for this stack and why "
    "you check it first, and the least-privilege or blast-radius principle behind the "
    "fix.\n"
)
...
_SHAPE_COMPARISON = (
    _NO_SENTENCES +
    "NO PROSE, ANYWHERE IN THIS SHAPE -- observed failing more here than in any other. A comparison badly wants to end in a summarising paragraph ('The real call hinges on whether your constraint is...'). Do not write it. Every line is a bullet, including the closing decision. If you are tempted to explain the trade-off in a sentence, that sentence IS the decision bullet -- compress it to 'LABEL -- why' and stop.\n"
...
_CATEGORY_WORD_LIMITS: dict[str, int] = {
    ...
    # 12-16 short area bullets (~7 words each) + 2 decision bullets lands ~150w.
    "scenario_troubleshooting": 200,
    "troubleshooting": 200,
